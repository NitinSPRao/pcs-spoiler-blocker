...
| LiveStats page — profile chart | Structure: `.profileBG.profileWrapper.xyProfileCont` > `.xyProfile` > SVG. SVG interior cannot be fully CSS-themed. **Approach: frame + soften** — style `.profileWrapper` as a dark card; apply `filter: brightness(0.82)` on `.xyProfile` to soften white without inverting chart colors. Leave `.kmdone`, `.profilePerc` at natural colors. |
