...
| LiveStats page — stats row + profile unified card | Common ancestor has no class/id. **Solution: `div:has(> ul.ls5b-kpi)`** — direct child combinator makes this specific (does NOT match `div.page-content`). Give this div `background: #ffffff`, `border: 1px solid #3a4556`, `border-top: 2px solid gold`, `border-radius: 4px`, `overflow: hidden`. `ul.ls5b-kpi` and `.profileWrapper` then need NO borders of their own. Gap between stats and chart is the body bg showing through — white parent covers it. Span labels: `color: #5a6070`, Barlow Condensed, uppercase. Div values: `color: #1a2030`, bold. |
