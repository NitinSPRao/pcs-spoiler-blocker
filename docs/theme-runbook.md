...
| LiveStats page — stats row + profile unified card | Common ancestor has no class/id. **Solution: `div:has(> ul.ls5b-kpi)`** — direct child combinator is specific enough (does NOT match `div.page-content`). Give this div `background: #ffffff`, `color: #1a2030` (overrides body light color so HTML text inside is readable), `border: 1px solid #3a4556`, `border-top: 2px solid gold`, `border-radius: 4px`. **Do NOT add `overflow: hidden`** — absolutely positioned children (race name chip, climb labels near edges) get clipped. Background is auto-clipped to `border-radius` without `overflow: hidden`. Also add `div:has(> ul.ls5b-kpi) text { fill: #1a2030 !important }` — SVG text inside the chart uses `fill` for color; `body { color: #e8edf3 }` makes it near-invisible on white without this fix. |
| LiveStats page — keypoint/climb labels | Class: `div.title.keypointTitle`. These elements are **outside `div:has(> ul.ls5b-kpi)` in the DOM** (appended to the chart container, not the stats/card div — confirmed because CSS inheritance from the card does not reach them). Do NOT set `color: #1a2030` on `.keypointTitle` — the elements live on the dark page background and dark text on dark = invisible. Let body color (`#e8edf3`) apply so they show as light text on dark. Also: do NOT set `color: #1a2030` on `div:has(> ul.ls5b-kpi)` itself — if the common ancestor is wide enough to contain the sidebar, the dark color cascades to sidebar nav items (dark text on dark sidebar = invisible). Stats row colors (`ul.ls5b-kpi li div/span`) are set explicitly and don't need the container color. |
