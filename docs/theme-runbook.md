...
| LiveStats page — stats row | `ul.ls5b-kpi` on dark page background. Stats labels (`ul.ls5b-kpi li span`): `color: #8896a8`. Stats values (`ul.ls5b-kpi li div`): `color: #e8edf3 !important; font-weight: 700`. Do NOT use dark colors here — the stats row has no white bg. |
| LiveStats page — keypoint/climb labels | Class: `div.title.keypointTitle`. Positioned on the **white chart sky** (`.xyProfile` has `background: white`). Set `color: #1a2030 !important` so dark text is readable on white. **Do NOT** rely on container inheritance — `.keypointTitle` elements are outside `ul.ls5b-kpi`'s DOM parent, confirmed by failed inheritance tests. The cascading issue with sidebar nav (dark text on dark bg) does NOT apply here because `.keypointTitle` is scoped to the chart area, not a wide container. |
