...
| LiveStats page — profile chart | Structure: `.profileBG.profileWrapper.xyProfileCont` > `.xyProfile` > SVG. SVG interior cannot be fully CSS-themed. **Approach: frame, don't invert** — style `.profileWrapper` as a dark card (`bg #1a2030`, `border #3a4556`, `border-radius`, `padding`, `overflow:hidden`). Leave `.xyProfile`, `.kmdone`, `.profilePerc` at natural light colors. |
