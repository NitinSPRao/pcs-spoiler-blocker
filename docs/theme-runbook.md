...
| Homepage LiveStats widget | `ul.hp3-livestats` (container), `li a` (card — bg on `a` not `li`), `span.status.live` (LIVE badge), `span.title` (race name), `div.togo` (km counter), `div.situ_txt` (status text). Progress bar div has **inline `background: linear-gradient(#fff → #c3db23)`** — leave unthemed to preserve race progress visualization. |
| LiveStats page — profile chart | Structure: `.profileBG.profileWrapper.xyProfileCont` > `.xyProfile` > SVG. White bg on `.xyProfile` and `.kmdone.profilePerc`. Override all: `.xyProfile, .kmdone, .profilePerc, .profileWrapper, .profileBG, .xyProfileCont`. |
| LiveStats page — detail profile | `div.detailProfileCont` (outer), `div.detailProfile` (inner chart), wrapping `div` has inline `border: 1px solid #ddd` — override with `.detailProfileCont > div { border-color }`. |
