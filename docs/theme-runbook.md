...
| LiveStats page — profile chart | Structure: `.profileBG.profileWrapper.xyProfileCont` > `.xyProfile` > SVG. SVG interior cannot be fully CSS-themed. **Approach: frame only** — style `.profileWrapper` as a dark card (`bg #1a2030`, `border #3a4556`, `border-radius`, `padding`, `overflow:hidden`). Leave `.xyProfile` and chart colors natural. No brightness filters. |
...
| LiveStats page — stats row | `ul.ls5b-kpi` (KM TO GO / RACETIME / KM DONE / AVG. etc). Each stat: `li > span` (label) + `div.kmtogo` / `div.racetime` etc (value). Apply Barlow Condensed + uppercase + letter-spacing to span labels; font-weight 700 to values. |
