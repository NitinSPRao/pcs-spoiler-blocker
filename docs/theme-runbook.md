...
| LiveStats page — stats row + profile unified card | Common ancestor of `ul.ls5b-kpi` and `.profileWrapper` has no class/id — cannot be targeted. **Workaround: split-card** — `ul.ls5b-kpi` gets top half (gold top border, dark bg, sides, no bottom, `border-radius: 4px 4px 0 0`); `.profileWrapper` gets bottom half (sides + bottom border, no top border, `border-radius: 0 0 4px 4px`). Together they appear as one unified card. Fonts on `ul.ls5b-kpi *`: Barlow Condensed, span labels uppercase + letter-spacing, div values bold. |
