...
| LiveStats page — profile chart | Structure: `div[position:absolute;bottom:0]` (no class) > `.profileBG.profileWrapper.xyProfileCont` > `.kmdone.profilePerc` (progress bar, inline width%) + `.xyProfile` (clip-path polygon for elevation silhouette) + SVG. `.profileWrapper` has inline `background: yellowgreen` — do NOT override, kills the progress bar. `.xyProfile` clip-path clips to the terrain area (NOT the sky), so the sky is transparent showing the parent container's background. **The chart sky is dark in dark mode because the parent container has no background.** Fix: `div:has(> .profileWrapper) { background: #ffffff !important }` — gives the nameless parent container a white background, making the sky appear white. **Do NOT add border to `.profileWrapper`** — `border-top: 2px solid #e8b400` on `.profileWrapper` draws a visible gold line inside the white card at the chart's top edge. The white card container (`div:has(> ul.ls5b-kpi)`) already provides the gold top border for the whole card. |
| LiveStats page — detail profile | `div.detailProfileCont` (outer). Dark card frame: `background: #1a2030`, `border: 1px solid #3a4556`, `border-radius: 4px`, `padding: 8px`. Wrapping `div` has inline `border: 1px solid #ddd` — override with `.detailProfileCont > div { border-color: #3a4556 }`. |
| LiveStats page — stats row + white card | `div:has(> ul.ls5b-kpi)` — set `background: #ffffff` **only**, NO `color` property. `background` does not inherit to children; sidebar's own `background: #1f2738` covers the white. Adding `color: #1a2030` cascades to sidebar text (dark on dark = invisible) — confirmed bug in earlier attempt. Stats labels: `ul.ls5b-kpi li span { color: #5a6070 }`. Stats values: `ul.ls5b-kpi li div { color: #1a2030; font-weight: 700 }`. Add gold top border + `border-radius: 4px`. |
| LiveStats page — keypoint/climb labels | Class: `div.title.keypointTitle`. Positioned on the **white chart sky** (sky area is the parent container of `.profileWrapper` with `background: #ffffff`). Set `color: #1a2030 !important` for dark text on white. Elements are outside `ul.ls5b-kpi`'s DOM parent (inheritance fails), so must target directly. |
| LiveStats page — race nav chips | Links to other live races at top-right of stats card. Class: `a.blue.fs10`. These are affected by the existing `.blue { background: #252d3a; color: #e8b400 }` rule which makes them dark on the white card. Fix: scoped override — `div:has(> ul.ls5b-kpi) a.blue { background: #f0f4f8; color: #1a2030; border: 1px solid #c8d0db }` — light grey bg with dark text for white-card context. |
