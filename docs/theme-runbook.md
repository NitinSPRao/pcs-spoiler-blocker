...
| LiveStats page — keypoint/climb labels | Class: `div.title.keypointTitle`. These are **absolutely positioned** elements — they do NOT inherit `color` from the parent `div:has(> ul.ls5b-kpi) { color: #1a2030 }`. Must target directly: `.keypointTitle, .keypointTitle * { color: #1a2030 !important }`. Without this, labels render in the body text color (`#e8edf3`) and are invisible on the white card background. |
